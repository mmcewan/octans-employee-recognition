      		...
      		res.send("RECEIVER USER NOT FOUND");
		    	...
		    	if (err)  {
					...
					var get_cloud_image = http.get(asignature, function(response) {
						...
						var latexStrings = ["\\documentclass[tikz, 18pt]{extarticle}", "\\usepackage{color}", "\\usepackage{tikz}", "\\usepackage[landscape,left=2cm,right=2cm,top=2cm,bottom=2cm]{geometry}", "\\usepackage[T1]{fontenc}", "\\usepackage{setspace}", "\\usepackage{graphicx}", "\\usepackage{eso-pic}", "\\newcommand \\BackgroundPic{\\put(0,0){\\parbox[b][\\paperheight]{\\paperwidth}{\\vfill \\centering \\includegraphics[height = \\paperheight, width = \\paperwidth]{" + backgroundfile +"} \\vfill}}}",  "\\begin{document}", "\\AddToShipoutPicture{\\BackgroundPic}", "\\pagenumbering{gobble}", "\\noindent", "\\makebox[\\textwidth][c]", "{\\begin{minipage}[c]{1.5\\textwidth}", "\\centering \\Huge \\color{red} Octans Group Company\\vskip0.8em \\large Corvallis, OR\\vskip0.8em \\large \\color{black} Employee Recognition Award \\vskip0.8em \\Huge \\color{red}" + typename + "\\vskip0.8em  \\large \\color{black}Awarded to: " + areceiver + "\\vskip0.8em  \\normalsize \\color{black}" + amessage + "\\vskip0.8em  \\normalsize \\color{black} Recognized by: \\color{black}" + agiver + " on " + adate + "\\par", "\\vskip0.8em \\begin{tikzpicture}[remember picture,overlay]\\node[inner sep=0pt] at (0,-2){\\includegraphics[width=4cm, height=4cm]{" + sigfilepath + "}} \\end{tikzpicture}", "\\end{minipage}}", "\\begin{tikzpicture}[remember picture,overlay]\\node[anchor=north east,inner sep=0pt] at ($(current page.north east) + (-2in,-2in)$){\\includegraphics[width=4cm, height=4cm]{" + logofile + "}} \\end{tikzpicture}","\\end{document}" ];
