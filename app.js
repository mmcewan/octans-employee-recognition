      		...
      		res.send("RECEIVER USER NOT FOUND");
		    	...
		    	if (err)  {
					...
					var get_cloud_image = http.get(asignature, function(response) {
						...
						var latexStrings = ["\\documentclass[tikz, 20pt]{extarticle}", "\\usepackage{color}", "\\usepackage{tikz}", "\\usepackage[landscape,left=2cm,right=2cm,top=2cm,bottom=2cm]{geometry}", "\\usepackage[T1]{fontenc}", "\\usepackage{setspace}", "\\usepackage{graphicx}", "\\usepackage{eso-pic}", "\\newcommand \\BackgroundPic{\\put(0,0){\\parbox[b][\\paperheight]{\\paperwidth}{\\vfill \\centering \\includegraphics[height = \\paperheight, width = \\paperwidth]{" + backgroundfile +"} \\vfill}}}",  "\\begin{document}", "\\AddToShipoutPicture{\\BackgroundPic}", "\\pagenumbering{gobble}", "\\noindent", "\\makebox[\\textwidth][c]", "{\\centering \\Huge \\color{red} Octans Group Company " + "\\begin{tikzpicture}[remember picture,overlay]\\node[sep=0pt]{\\includegraphics[width=2cm, height=2cm]{" + logofile + "}} \\end{tikzpicture}", "\\vskip0.8em \\large Corvallis, OR\\vskip0.8em \\large \\color{black} Employee Recognition Award \\vskip0.8em \\Huge \\color{red}" + typename + "\\vskip0.8em  \\large \\color{black}Awarded to: " + areceiver + "\\vskip0.8em  \\normalsize \\color{black}" + amessage + "\\vskip0.8em  \\normalsize \\color{black} Recognized by: \\color{black}" + agiver + "\\par", "\\vskip0.8em \\begin{tikzpicture}[remember picture,overlay]\\node[inner sep=0pt] at (0,-2){\\includegraphics[width=4cm, height=4cm]{" + sigfilepath + "}} \\end{tikzpicture} \\vskip0.8em" + adate + "\\end{document}" ];
